...
export function calculateTotalRevenue(proforma: Proforma): number {
  const unitMixRevenue = proforma.unitMix.reduce((total, unitType) => {
    return (
      total +
      unitType.units.reduce((unitTypeTotal, unit) => {
...
export function calculateProformaMetrics(proforma: Proforma): Proforma {
  const totalProjectCostInclFinancing =
    proforma.totalProjectCostInclFinancing ||
    proforma.totalExpenses +
  ...
  const roiFormula =
    proforma.sources.equityPct && totalProjectCostInclFinancing
      ? grossProfit /
        ((proforma.sources.equityPct / 100) * totalProjectCostInclFinancing)
      : 0;
  const annualizedRoi =
    roiFormula && proforma.projectLength
...
export function saveProforma(projectId: string, proforma: Proforma): void {
  ...
  sessionStorage.setItem(
    `${PROFORMAS_KEY}_${projectId}`,
    JSON.stringify(proformas)
  );
}
...
export function deleteProforma(projectId: string, proformaId: string): void {
  ...
  sessionStorage.setItem(
    `${PROFORMAS_KEY}_${projectId}`,
    JSON.stringify(filteredProformas)
  );
}
...
export function createNewProforma(
  projectId: string,
  projectLandCost: number,
  proformaType: string
): Proforma {
  return {
