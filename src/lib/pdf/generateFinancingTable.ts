...
type TableCell =
  ...
  | {
      content: string;
      colSpan?: number;
      styles?: { fontStyle?: "bold" | "normal"; halign?: "left" | "right" };
    };
type TableRow = TableCell[];

export function generateFinancingTable(
  doc: jsPDF,
  proforma: Proforma,
  ...
  const debtGBA = "73.78";
  const debtUnit = "262,317";
  const debtPctRevenue = "55%";
  ...
  const financingTableBody: TableRow[] = [
    [
      ...
      proforma.totalExpenses
        ? formatCurrency(proforma.totalExpenses - totalFinancing)
        : formatCurrency(0),
    ],
    [
      "Interest Cost",
    ...
    didDrawCell: function (data: CellHookData) {
      ...
      if (
        ...
        if (
          ...
          doc.line(
            tableLeftX,
            data.cell.y + (data.cell.height || 0),
            tableRightX,
            data.cell.y + (data.cell.height || 0)
          );
        }
  ...
  doc.text(
    proforma.totalProjectCostInclFinancing
      ? proforma.totalProjectCostInclFinancing.toLocaleString()
      : "0",
    520,
    y + 12,
    { align: "right" }
